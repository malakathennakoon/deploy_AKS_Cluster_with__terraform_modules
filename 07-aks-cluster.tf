...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  name                 = "${module.resource_group.rg_name_out}-cluster"
  location             = var.location
  resource_group_name  = module.resource_group.rg_name_out
  dns_prefix           = "${module.resource_group.rg_name_out}-cluster"
  kubernetes_version   = data.azurerm_kubernetes_service_versions.current.latest_version
  node_resource_group  = "${module.resource_group.rg_name_out}-node-rg"
  ...
  default_node_pool {
    ...
    vnet_subnet_id       = module.subnet.azure_subnet_id
    node_labels = {
